        ...
        stage('发布项目') {
          steps {
            script {
              ...
              sh "ssh -o StrictHostKeyChecking=no ${deployNode} \"tar -C ${deployDir}.tmp/ -zxvf ${deployDir}.tmp/ROOT-saas-plus-gitbook.tar.gz && mv  ${deployDir}.tmp/_book/*  ${deployDir}.tmp/ && rm -rf ${deployDir}.tmp/_book/ ${deployDir}.tmp/ROOT-saas-plus-gitbook.tar.gz \""
